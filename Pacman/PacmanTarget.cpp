...
bool PacmanTarget::BFS(const Board& board, const Point& oldCoord, Point& newCoord) const
{
	...
	vector<int> possibleDirectionsColumnAdder = getNewIntegerOfDirectionsVector(), possibleDirectionsRowAdder = getNewIntegerOfDirectionsVector();
	rotate(possibleDirectionsColumnAdder.begin(), next(possibleDirectionsColumnAdder.begin()), possibleDirectionsColumnAdder.end());
	...
	while (currCoord.empty() == false)
	{
		int currX = currCoord.front().getX(), currY = currCoord.front().getY();
		currCoord.pop();
		for (int i = 0; i < valOf(entityConstants::NUM_OF_ARROWKEYS); i++)
		{
...
vector<int> PacmanTarget::getNewIntegerOfDirectionsVector() const
{
